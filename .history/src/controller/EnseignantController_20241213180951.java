...
public class EnseignantController {
    ...
    private void enregistrerSeance() {
        ...
        String checkQuery = "SELECT COUNT(*) FROM seances WHERE classe = ? AND matiere = ? AND jour = ? AND heure = ? AND matricule_enseignant = ?";
        try (Connection connection = DatabaseConnection.getConnection();
                PreparedStatement checkStatement = connection.prepareStatement(checkQuery)) {

            checkStatement.setString(1, classe);
            checkStatement.setString(2, matiere);
            checkStatement.setString(3, jour);
            checkStatement.setString(4, heure);
            checkStatement.setString(5, matricule);

            ResultSet checkResult = checkStatement.executeQuery();
            if (checkResult.next() && checkResult.getInt(1) > 0) {
                showAlert(Alert.AlertType.WARNING, "Erreur", "Cette séance existe déjà.");
                return; // Ne pas enregistrer la séance si elle existe déjà
            }
        } catch (SQLException e) {
            showAlert(Alert.AlertType.ERROR, "Erreur", "Problème lors de la vérification : " + e.getMessage());
            return;
        }

        // Si la séance n'existe pas, l'enregistrer
        ...
        try (Connection connection = DatabaseConnection.getConnection();
            ...
            if (rowsInserted > 0) {
                ...
                afficherSeances(); // Actualiser l'affichage des séances
            }
